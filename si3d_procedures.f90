...
!-------------------------------------------------------------------------
   ...
   CASE DEFAULT
     ...
     END DO
     PRINT*, " Primera fila init.txt: ", Scalardepthile(1,:)
...
!-----------------------------------------------------------------------
   ...
   j=jsbcNBO(1)
   do i=isbcNBO(1),iebcNBO(1)
       l=ij2l(i,j)
       !print *,"ex1",sum(ex(:,l)),i,j
   end do
   !print *,"***********************/" !mario
   do i=isbcNBO(1),iebcNBO(1)
       l=ij2l(i,j-1)
...
!!   END DO
   ...
   CALL matcon(t_matcon2,Bstart,Bend,lWCH,lSCH,Beagx,Bearx,Beagy,Beary,Bsx,Bsy,Bdd,Bqq,Brr,Bagx,Bagy,Barx,Bary)
   !print *,"hihihi10:",omp_get_thread_num()
   !$omp barrier
   j=jsbcNBO(1)
   do i=isbcNBO(1),iebcNBO(1)
       l=ij2l(i,j)
       !print *,"ex2",sum(ex(:,l)),i,j
       !print *,"Bagx",sum(Bagx(:,l)),i,j
       !print *,"Bagy",sum(Bagy(:,l)),i,j
       !print *,"Barx",sum(Barx(:,l)),i,j
       !print *,"Bary",sum(Bary(:,l)),i,j
       !print *,"Beagx",Beagx(l),i,j
       !print *,"Beagy",Beagy(l),i,j
       !print *,"Bearx",Bearx(l),i,j
       !print *,"Beary",Beary(l),i,j
       !print *,"Bqq",Bqq(l),i,j
       !print *,"Bdd",Bdd(l),i,j
       !print *,"Brr",Brr(l),i,j
   end do
...
!!   end if
   ...
   do i=isbcNBO(1),iebcNBO(1)
       l=ij2l(i,j)
       !print *,"uh",sum(uh(:,l)),i,j
       !print *,"vh",sum(vh(:,l)),i,j
   end do
   !print *,"***********************/"  ! mario
   do i=isbcNBO(1),iebcNBO(1)
       l=ij2l(i,j-1)
...
!-----------------------------------------------------------------------
   ...
   REAL :: sumadvx,sumcorx,sumhdx,sumadvx2,sumcorx2,sumhdx2
      ...
      DO liter = lhiW(omp_get_thread_num ( )+1), lhf(omp_get_thread_num ( )+1)
        ...
        ex(kmx+1:km1,l) = 0.0; 
         ...
         DO k = k1x,kmx
            ...
            ! Bathy rotada:R2 con smagorinski:
            !IF (i>510 .AND. i<545 .AND. j>170 .AND. j<210) THEN
            !  hdx = 0.2*hdx
            !ENDIF
            ...
            ! Aumento difusividad con el grid de 5m 
            ! IF ((i<=53                .AND. j<=230                 ))  ! .OR. & !CINTIA (SUTTER GEO)
            !THEN  !CINTIA (SUTTER DCC)   
            !  !corx = 0.0
            !  !advx = 0.0
            !  hdx = 2.*hdx
            !ENDIF
            ...
            !  corx  = 0.
            !ENDIF
            !print *,"iiii",i,j,jsbc(1),isbc(1),iebc(1)
            !if(j .EQ. jsbcNBO(1)-1 .AND. i <= iebcNBO(1) .AND. i >= isbcNBO(1)) THEN
                !print *,"aqui no entro"
                !print *,"aadvx:",advx
                !print *,"iiadv:",iadv
                !print *,"ccorx:",corx
            ...
            !end if
            !if(j .EQ. jsbcNBO(1) .AND. i <= iebcNBO(1) .AND. i >= isbcNBO(1)) THEN
                !print *,"aqui no entro"
                !print *,"aadvx:",advx
                !print *,"iiadv:",iadv
                !print *,"ccorx:",corx
                !print *,"hhdx:",hdx
                !sumadvx2=sumadvx+advx
                !sumcorx2=sumcorx+corx
                !sumhdx2=sumhdx+hdx
        ...
        !        print *,"aadvx:",sumadvx2
        !        print *,"iiadv:",iadv
        !        print *,"ccorx:",sumcorx2
        !        print *,"hhdx:",sumhdx2
        !        sumadvx2=0
        !        sumcorx2=0
        !        sumhdx2=0
   ...
   CASE (1)
      ...
      DO liter = lhiWCE(omp_get_thread_num ( )+1), lhfCE(omp_get_thread_num ( )+1)
            ...
            taubx = cd*SQRT((uhpp(kmx,     l)*uhpp(kmx,        l)) +          &
                  ...
                  &                /(hup(kmx,l)*hup(kmx,l))
            !-------------------------------------------------------------------
            ! Increasing the bottom roughness at GEO dx = 5x5 m
            !IF ((i>=613                 .AND. j<=27                 ) .OR. &
            !     (i>=615                .AND. j==28                 ) .OR. &
            !     (i>=617                .AND. j==29                 ) .OR. &
            !     (i>=619                .AND. j==30                 ) .OR. &
            !     (i>=621                .AND. j==31                 )) THEN       !CINTIA (GEO)
            !                                                            
            !            cd = 0.006                                          !CINTIA  
            !            !print *,"cd_geo:",cd
            !ENDIF
            ...
            ! Increasing the bottom roughness at GEO dx = 5x5 m
            !IF  ((i>=95                 .AND. j<=182                ) .OR. &
            !     (i>=200                .AND. j<=197                ) .OR. &
            !     (i>=211                .AND. j<=205                ) .OR. &
            !     (i>=207                .AND. j<=199                )) THEN       !CINTIA (GEO)
            !                                                            
            ! 
            !   taubx = 0.02*SQRT((uhpp(kmx,     l)*uhpp(kmx,        l)) +          &
            !      &  ((0.25*(vhpp(kmx,lEC(l))+vhpp(kmx,        l)             &
            !      &         +vhpp(kmx,lSC(l))+vhpp(kmx,lSC(lEC(l)))))**2))    &
            !      &                /(hup(kmx,l)*hup(kmx,l))
            ! 
            ...
            ! Increasing the bottom roughness at GEO dx = 2x2 m
           !IF  ((i>=219                 .AND. j<=121                ) .OR. &
           !      (i>=230                 .AND. j<=131                ) .OR. &
           !      (i>=234                 .AND. j<=135                ) .OR. &
           !      (i>=238                 .AND. j<=143                ) .OR. &
           !      (i>=252                 .AND. j<=155                ) .OR. &
           !      (i>=268                 .AND. j<=164                ) .OR. &
           !      (i>=278                 .AND. j<=168                )) THEN       !CINTIA (GEO)
            !IF  ((i>=219+135                 .AND. j<=121                ) .OR. &
            !     (i>=230+135                 .AND. j<=131                ) .OR. &
            !     (i>=234+135                 .AND. j<=135                ) .OR. &
            !     (i>=238+135                 .AND. j<=143                ) .OR. &
            !     (i>=252+135                .AND. j<=155                ) .OR. &
            !     (i>=268+135                 .AND. j<=164                ) .OR. &
            !     (i>=278+135                 .AND. j<=168                )) THEN       !CINTIA (GEO)
            !                                                            
            ! 
            !   taubx = 0.02*SQRT((uhpp(kmx,     l)*uhpp(kmx,        l)) +          &
            !      &  ((0.25*(vhpp(kmx,lEC(l))+vhpp(kmx,        l)             &
            ...
            ! Increasing the bottom roughness at DCC dx = 2x2 m
            !IF  ((i>=549                 .AND. j<=569                ) .OR. &
            !    (i>=546                .AND. j<=562                ) .OR. &
            !     (i>=542                .AND. j<=552                ) .OR. &
            !      (i>=537                .AND. j<=544                ) .OR. &
            !      (i>=533                .AND. j<=539                ) .OR. &
            !     (i>=531                .AND. j<=537                )) THEN       !CINTIA (DCC)
            !IF  ((i>=549+135                  .AND. j<=569                ) .OR. &
            !     (i>=546+135                 .AND. j<=562                ) .OR. &
            !     (i>=542+135                 .AND. j<=552                ) .OR. &
            !      (i>=537+135                 .AND. j<=544                ) .OR. &
            !      (i>=533+135                .AND. j<=539                ) .OR. &
            !     (i>=531+135                 .AND. j<=537                )) THEN       !CINTIA (DCC)                                                            
            ! 
            !   taubx = 0.02*SQRT((uhpp(kmx,     l)*uhpp(kmx,        l)) +          &
            !      &  ((0.25*(vhpp(kmx,lEC(l))+vhpp(kmx,        l)             &
            !      &         +vhpp(kmx,lSC(l))+vhpp(kmx,lSC(lEC(l)))))**2))    &
            ...
            !if(j .EQ. jsbc(1)-1 .AND. i <= iebc(1) .AND. i >= isbc(1)) THEN
                !print *,"aqui no entro"
      ...
      !.....End loop over u-pts.....
      END DO
      CALL MODmatmom4openBCX(Bstart,Bend,Beagx,Bearx,Bagx,Barx) !MAC NESTING
   ...
   CASE (2)
      ...
      DO liter = lhiCN(omp_get_thread_num ( )+1), lhfCN(omp_get_thread_num ( )+1)
            ...
            tauby = cd*SQRT((vhpp(kmy,l)*vhpp(kmy,l)) +                       &
                  ...
                  &         /(hvp(kmy,l)*hvp(kmy,l))
            !-------------------------------------------------------------------
            ! Increasing the bottom roughness at GEO 
            !IF ((i>=613                 .AND. j<=27                 ) .OR. &
            !     (i>=615                .AND. j==28                 ) .OR. &
            !     (i>=617                .AND. j==29                 ) .OR. &
            !     (i>=619                .AND. j==30                 ) .OR. &
            ...
            !                                                            
            ! 
            !   tauby = 0.02*SQRT((vhpp(kmy,l)*vhpp(kmy,l)) +                       &
            !      &  ((0.25*(uhpp(kmy,lNC(l))+uhpp(kmy,lWC(lNC(l)))           &
            !      &         +uhpp(kmy,lWC(l))+uhpp(kmy,l)))**2))              &
            !      &         /(hvp(kmy,l)*hvp(kmy,l))
            !
	...
	ENDIF
         ...
         Call MODmatmom4openBCY(Bstart,Bend,Beagy,Beary,Bagy,Bary) !MAC NESTING
...
!***********************************************************************
SUBROUTINE matcon(t_matcon2,Bstart,Bend,lWCH,lSCH,Beagx,Bearx,Beagy,Beary,Bsx,Bsy,Bdd,Bqq,Brr,Bagx,Bagy,Barx,Bary)
!***********************************************************************
!
...
!-----------------------------------------------------------------------
   ...
   !IF (nopen > 0 ) CALL MODmatcon(Bstart,Bend,Beagx,Beagy,Bearx,Beary,Bagx,Bagy,Barx,Bary)
...
!     i = l2i(l); j = l2j(l);
   ...
   !.....Adjust qq & sx/sy matrices for open boundary conditions.....
   IF (nopen > 0 ) CALL MODqqddrr4openBC(Bstart,Bend,Bqq,Brr,Bsx,Bsy,Bdd,Beagx,Beagy,Bearx,Beary,lWCH,lSCH)
...
!-----------------------------------------------------------------------
   ...
   !.....Timing.....
   REAL, EXTERNAL :: TIMER
   REAL :: btime, etime, auxcoeff1,auxcoeff2,auxcoeff3,auxcoeff4,auxcoeff5,auxrhs,auxzeta
   btime = TIMER(0.0)
...
!     j = l2j(m);
   ...
   j=l2j(m)
     if(j<=28) THEN
        auxcoeff1 = auxcoeff1 + coeffA(m,1)
        auxcoeff2 = auxcoeff2 + coeffA(m,2)
        auxcoeff3 = auxcoeff3 + coeffA(m,3)
        auxcoeff4 = auxcoeff4 + coeffA(m,4)
        auxcoeff5 = auxcoeff5 + coeffA(m,5)
        auxrhs = auxrhs + rhs(m)
        auxzeta = auxzeta + zeta(m)
     end if
        
   !print *,"coeff1:",sum(coeffA(:,1)),"coeff2:",sum(coeffA(:,2)),"coeff3:",sum(coeffA(:,3))
   !print *,"coeff4:",sum(coeffA(:,4)),"coeff5:",sum(coeffA(:,5))
   !print *,"rhs:",sum(rhs(:)),"zeta:",sum(zeta(:))
   end do
   do i=1,im
      do j=1,jm
         if(j<=28) THEN
         if(mask2d(i,j)) THEN
             m=ij2l(i,j)
             !print *,"coeff1:",coeffA(m,1),"coeff2:",coeffA(m,2),"coeff3:",coeffA(m,3)
             !print *,"coeff4:",coeffA(m,4),"coeff5:",coeffA(m,5),"m:",m
             !print *,"i:",i,"j:",j
             !print *,"rhs:",rhs(m),"zeta:",zeta(m)
         end if
         end if
      end do
   end do
   !$omp barrier
  ! print *,"coeff1:",auxcoeff1,"coeff2:",auxcoeff2,"coeff3:",auxcoeff3
  ! print *,"coeff4:",auxcoeff4,"coeff5:",auxcoeff5
  ! print *,"rhs:",auxrhs,"zeta:",auxzeta
   IF(omp_get_thread_num ( )==0) THEN
    ...
    SELECT CASE ( iside(no) )
         ...
         CASE (1)
            ...
            DO j = js, je
               ...
               sEBpp(:,j) = sEBp(:,j);   sWBpp(:,j) = sWBp(:,j);  !MAC NESTING
               sEBp(:,j)  = (sEBpp(:,j) + sEB(:,j))/2.; sWBp(:,j)  = (sWBpp(:,j) + sWB(:,j))/2.;
               auxhEBpp(:,j) = auxhEBp(:,j);   auxhWBpp(:,j) = auxhWBp(:,j);
               auxhEBp(:,j)  = (auxhEBpp(:,j) + auxhEB(:,j))/2.; auxhWBp(:,j)  = (auxhWBpp(:,j) + auxhWB(:,j))/2.;

               uhNBpp(:,j) = uhNBp(:,j); huNBpp(:,j) = huNBp(:,j);
               uhSBpp(:,j) = uhSBp(:,j); huSBpp(:,j) = huSBp(:,j);
               uhNBp(:,j)  = (uhNBpp(:,j) + uhNB(:,j))/2.; huNBp(:,j) = (huNBp(:,j)+huNB(:,j))/2.;
               uhSBp(:,j)  = (uhSBpp(:,j) + uhSB(:,j))/2.; huSBp(:,j) = (huSBp(:,j)+huSB(:,j))/2.;
               sNBpp(:,j) = sNBp(:,j);   sSBpp(:,j) = sSBp(:,j);   !MAC NESTING
               sNBp(:,j)  = (sNBpp(:,j) + sNB(:,j))/2.; sSBp(:,j)  = (sSBpp(:,j) + sSB(:,j))/2.;
               auxhNBpp(:,j) = auxhNBp(:,j);   auxhSBpp(:,j) = auxhSBp(:,j);
               auxhNBp(:,j)  = (auxhNBpp(:,j) + auxhNB(:,j))/2.; auxhSBp(:,j)  = (auxhSBpp(:,j) + auxhSB(:,j))/2.;
            END DO
         ...
         CASE (2)
            ...
            DO i = is, ie
               ...
               vhEBpp(:,i) = vhEBp(:,i); hvEBpp(:,i) = hvEBp(:,i);
               vhWBpp(:,i) = vhWBp(:,i); hvWBpp(:,i) = hvWBp(:,i);
               vhEBp(:,i)  = (vhEBpp(:,i) + vhEB(:,i))/2.; hvEBp(:,i) = (hvEBp(:,i)+hvEB(:,i))/2.;
               vhWBp(:,i)  = (vhWBpp(:,i) + vhWB(:,i))/2.; hvWBp(:,i) = (hvWBp(:,i)+hvWB(:,i))/2.;
               sEBpp(:,j) = sEBp(:,j);   sWBpp(:,j) = sWBp(:,j);  !MAC NESTING
               sEBp(:,j)  = (sEBpp(:,j) + sEB(:,j))/2.; sWBp(:,j)  = (sWBpp(:,j) + sWB(:,j))/2.;
               auxhEBpp(:,j) = auxhEBp(:,j);   auxhWBpp(:,j) = auxhWBp(:,j);
               auxhEBp(:,j)  = (auxhEBpp(:,j) + auxhEB(:,j))/2.; auxhWBp(:,j)  = (auxhWBpp(:,j) + auxhWB(:,j))/2.;
    ...
    SELECT CASE ( iside(no) )
         ...
         CASE (1)
            ...
            DO j = js, je
               ...
               sWBp(:,j)   = sWB(:,j)  ; sEBp(:,j)  = sEB(:,j);  !MAC NESTING
               auxhWBp(:,j)   = auxhWB(:,j)  ; auxhEBp(:,j)  = auxhEB(:,j);

               uhNBp(:,j)  = uhNB(:,j) ; huNBp(:,j) = huNB(:,j) ;
               uhSBp(:,j)  = uhSB(:,j) ; huSBp(:,j) = huSB(:,j) ;
               sNBp(:,j)   = sNB(:,j)  ; sSBp(:,j)  = sSB(:,j);  !MAC NESTING
               auxhNBp(:,j)   = auxhNB(:,j)  ; auxhSBp(:,j)  = auxhSB(:,j);
            END DO
         ...
         CASE (2)
            ...
            DO i = is, ie
               ...
               vhEBp(:,i)  = vhEB(:,i) ; hvEBp(:,i) = hvEB(:,i) ;
               vhWBp(:,i)  = vhWB(:,i) ; hvWBp(:,i) = hvWB(:,i) ;
               sWBp(:,j)   = sWB(:,j)  ; sEBp(:,j)  = sEB(:,j);  !MAC NESTING
               auxhWBp(:,j)   = auxhWB(:,j)  ; auxhEBp(:,j)  = auxhEB(:,j);
            END DO
...
!-----------------------------------------------------------------------
   ...
   !.....Local variables.....
   INTEGER :: i,j,k,l,kms,kmy,kmx,lol,liter,aux_indice,ide_t
   REAL    :: wght, wghtpp

   ide_t = omp_get_thread_num ( )+1 ! Mario
   !....Zeta array.....
