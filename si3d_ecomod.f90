...
!-----------------------------------------------------------------------
  ...
  !. . . .Local Variables
  CHARACTER(LEN=12) :: sdiofile="si3d_st.txt"
  CHARACTER(LEN=18) :: iosdfmt
  INTEGER:: ios, itr ,idep, istat
  INTEGER, PARAMETER :: niosd = 4
  ...
  ALLOCATE ( alpha(ntr), mexp(ntr), taucr(ntr), vdep(ntr), STAT=istat)
  ...
  DO itr = 1, ntr
    READ (UNIT=i99, FMT=iosdfmt, IOSTAT=ios) &
         alpha(itr), mexp(itr), taucr(itr), vdep(itr)
    PRINT *, itr, alpha(itr), mexp(itr), taucr(itr), vdep(itr)
    IF (ios /= 0) CALL input_error ( ios, 91 ) 
  END DO
...
!-----------------------------------------------------------------------
   ...
   ! ... Local variables
   INTEGER :: i, j, k, l, k1s, kms, itr, kmyp, kmym, kmxp, kmxm, liter
   REAL    :: r, zt, taub, ubott, vbott, tau0
   ...
   DO itr = 1, ntr
     ...
     DO liter = lhi(omp_get_thread_num ( )+1), lhf(omp_get_thread_num ( )+1)
       ...
       ! ....Compute bottom shear stress (cd*U^2)
       ubott = (uhp(kmxp,l)+uhp(kmxm,lWC(l)))/2./hp(kms,l)      
       vbott = (vhp(kmyp,l)+vhp(kmym,lSC(l)))/2./hp(kms,l)      
       taub  = cd * (rhop(kms,l)+1000.) * (ubott**2.+vbott**2.)

       ! ... Erosion flux E (kg/m2/s) at bottom cell
       tau0 = MAX(1.0E0, taucr(itr))
       sourcesink(kms,l,itr)= alpha(itr)*((taub-taucr(itr))/tau0)**mexp(itr)
...
!-----------------------------------------------------------------------
   ...
   ! ... Local variables
   INTEGER :: i, j, k, l, k1s, kms, itr, liter
   REAL    :: r, zt
