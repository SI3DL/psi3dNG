...
!------------------------------------------------------------------------
   ...
   IF (ios /= 0) CALL input_error ( ios, 7 )
   READ (UNIT=i5, FMT='(14X,I20)', IOSTAT=ios) ptr ! abh
   IF (ios /= 0) CALL input_error ( ios, 7 )
...
!------------------------------------------------------------------------
   ...
   IF (nopen > 0) THEN
     ALLOCATE(  uhWB  (km1,jm1), uhEB  (km1,jm1),   &
                ...
                sWBp (km1,jm1), sEBp (km1,jm1),   &   !MAC NESTING
                sSBp (km1,im1), sNBp (km1,im1),   &
                sWBpp (km1,jm1), sEBpp (km1,jm1),   &
                sSBpp (km1,im1), sNBpp (km1,im1),   &
                sWB (km1,jm1), sEB (km1,jm1),   &
                sSB (km1,im1), sNB (km1,im1),   &
                auxhWB (km1,jm1), auxhEB (km1,jm1),   &
                auxhSB (km1,im1), auxhNB (km1,im1),   &
                auxhWBp (km1,jm1), auxhEBp (km1,jm1),   &
                auxhSBp (km1,im1), auxhNBp (km1,im1),   &
                auxhWBpp (km1,jm1), auxhEBpp (km1,jm1),   &
                auxhSBpp (km1,im1), auxhNBpp (km1,im1),   &
                agWB (km1,jm1), agEB (km1,jm1),   &
                agSB (km1,im1), agNB (km1,im1),   &
                arWB (km1,jm1), arEB (km1,jm1),   &
                arSB (km1,im1), arNB (km1,im1),   &
   ...
   ! .... Allocate arrays used to store velocity boundary conditions
   IF (nopen > 0) THEN
     ALLOCATE(  uhSB  (km1,jm1), uhNB  (km1,jm1),   &
                huSB  (km1,jm1), huNB  (km1,jm1),   &
                vhEB  (km1,im1), vhWB  (km1,im1),   &
                hvEB  (km1,im1), hvWB  (km1,im1),   &
                uhSBpp(km1,jm1), uhNBpp(km1,jm1),   &
                huSBpp(km1,jm1), huNBpp(km1,jm1),   &
                vhEBpp(km1,im1), vhWBpp(km1,im1),   &
                hvEBpp(km1,im1), hvWBpp(km1,im1),   &
                uhSBp (km1,jm1), uhNBp (km1,jm1),   &
                huSBp (km1,jm1), huNBp (km1,jm1),   &
                vhEBp (km1,im1), vhWBp (km1,im1),   &
                hvEBp (km1,im1), hvWBp (km1,im1), STAT=istat )
     IF (istat /= 0) CALL allocate_error ( istat, 9 )
   ENDIF
...
!------------------------------------------------------------------------
   ...
   ALLOCATE ( Av  (km1,lm1), Dv  (km1,lm1),Dvm  (km1,lm1),            & !Andrea
...
!-----------------------------------------------------------------------
  ...
  INTEGER :: wind_id
  INTEGER :: i, j, k, ios, istat, n_frames, k_out, m1,m2, kp, kb, c,l
...
!     ENDIF
     ...
     DO m1 = 1, n_section_cells (k)
       ...
       DO m2 = k1, kmz(l)
         ...
         out_array(k_out,5) = uhp (m2,l) ! Changed 12/2010 SWA uhp
         out_array(k_out,6) = vhp (m2,l) ! Changed 12/2010 SWA vhp
...
!     ENDIF
     ...
     DO m1 = 1, n_section_cells(k)
       ...
       DO m2 = k1, kmz(l)
         k_out = k_out + 1
         out_array(k_out,1) = uhp (m2,l) ! Changed 12/2010 SWA uhp
         out_array(k_out,2) = vhp (m2,l) ! Changed 12/2010 SWA vhp
...
!-----------------------------------------------------------------------
   ...
   INTEGER :: i , j , k , l, kmx, kmy, js, je, is, ie, icl, lv, le
   ...
   DO nn = 1, nxNBO
     ...
     ipts = iptNBO(nn)
     ALLOCATE( outvar ( ipts, 11+ntr ), STAT=istat )
     ...
     SELECT CASE (isdNBO(nn))
       ...
       CASE (1,3)
         ...
         DO j = js,je
           ...
           le  = lv; IF (isdNBO(nn)== 3) THEN; le = ij2l(i+1,j); ENDIF 
           ...
           DO jj = jjs, jje 
             DO k = k1, kmx
               ...
               outvar(icl,6) = vh (k,le) 
               outvar(icl,7) = s (le) 
               outvar(icl,8) = hu(k,lv) 
               outvar(icl,9) = hv(k,le) 
               outvar(icl,10) = agx(k,lv)
               outvar(icl,11) = arx(k,lv)  
               
               IF (ntr > 0) THEN
                 outvar(icl,12:11+ntr) = tracer(k,l,1:ntr)
               ENDIF
               uflow = uflow + uh(k,lv)
             ENDDO
           ENDDO         
       ...
       CASE (2,4)
         ...
         DO i = is,ie
           ...
           le  = lv; IF (isdNBO(nn)== 2) THEN; le = ij2l(i,j+1); ENDIF 
           ...
           DO ii = iis, iie
             ...
             DO k = k1, kmy
               ...
               IF (icl > ipts) THEN
                 ...
                 STOP
               ENDIF 
               !print *,"agy:",agy(k,lv),"ary:",ary(k,lv),k,lv
               ...
               outvar(icl,5) = sal(k,l) 
               outvar(icl,6) = uh (k,le) 
               outvar(icl,7) = s (le) 
               outvar(icl,8) = hu(k,le) 
               outvar(icl,9) = hv(k,lv) 
               outvar(icl,10) = agy(k,lv)
               outvar(icl,11) = ary(k,lv) 
               IF (ntr > 0) THEN
                 outvar(icl,12:11+ntr) = tracer(k,l,1:ntr)
               ENDIF
               uflow = uflow + vh(k,lv)
     ...
     IF((MOD(n,MAX(ioNBO,1)) == 0)) THEN 
       IF (n == 0 ) THEN     
         WRITE(nboid) thrs,((outvar(m1,m2),m2=1,11+ntr),m1=1,ipts)
       ELSE
         WRITE(nboid) thrs,((outvar(m1,m2),m2=4,11+ntr),m1=1,ipts)
       END IF
     ENDIF
   ...
   IF( n == 0 ) THEN
     ...
     DO k_t = 1, ntr
        ...
        DO c=1,cm1
             ...
             out_array(k_out,5) = tracer(k,l,k_t)
             END DO;
        END DO
        ! ... Id # for plane file
   ...
   !ELSEIF (n>=475200) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=4s)
     ...
     DO k_t = 1, ntr
        ...
        DO c=1,cm1
             ...
             out_array(k_out,2) = tracer(k,l,k_t)
             END DO;
        END DO
        ! ... Id # for plane file
        tracer_id = tracer_id0 + k_t
...
!     ENDIF
     ...
     DO c=1,cm1
       ...
       DO k = k1, kmz(l)
         ...
         out_array(kout,10) = s (l)   !Cintia incorporamos wse
       END DO;
     END DO
     ! ... Print time stamp followed by the records
...
!     ENDIF
     ...
     DO c=1,cm1
       ...
       DO k = k1, kmz(l)
         ...
         out_array(kout,5) = Dvm(k,l)
         out_array(kout,6) = sal (k,l)
         out_array(kout,7) = s (l)
       END DO;
     END DO
     ! ... Print time stamp followed by the records
     WRITE(ptrack_id) n,year_out,mon_out, day_out,hour_out,  &
...
!***********************************************************************
SUBROUTINE outp2(n) ! ABh
!***********************************************************************
!
!  Purpose: To write the complete solution in the computational domain
!           The resulting file is used to drive particle tracking
!           simulations with PTRACK-TOOL. Write one file per output in 
!           subfolder /ptrack_hydro
!
! Andrea Hoyer 2013, Dec 16
!-----------------------------------------------------------------------

   INTEGER, INTENT(IN) :: n

   !.....Local variables.....
   CHARACTER(LEN=15):: path='./ptrack_hydro/'  ! ABH 2012 Feb 08
   CHARACTER (LEN = 20) :: ptrack_file  ! changed from 16 to 17 to include numbering
   INTEGER, PARAMETER   :: ptrack_id = 1002
   INTEGER              :: i, j, k, l, ios, istat, Noframes, m1, m2,  &
                           kout, year_out, day_out, mon_out, c
   REAL                 :: hour_out
...
!   REAL, ALLOCATABLE, DIMENSION(:,:) :: out_array
   ...
   IF( n == 0 ) THEN
     ...
     Dvm = 0.0 !Andrea
     n_ptrack = 0.0  ! initialize counter (abh 2013, Dec 10)

     PRINT*,' Writing first ptrack_hydro file --------------------------------- '

     ! ... Open output file & print data & initial conditions
     ptrack_file = "ptrack_hydro0.bnr"
     !OPEN(unit=ptrack_id,file=ptrack_file,FORM='UNFORMATTED',IOSTAT=ios)
     OPEN(unit=ptrack_id,file=path//ptrack_file,FORM='UNFORMATTED',IOSTAT=ios)   ! include path of folder where file shoulde be written
     IF(ios /= 0) THEN
       PRINT *, "Error opening ptrack hydro file = ", ios
       STOP
     ENDIF
...
!     DEALLOCATE (out_array)
 ...
 ELSE
     ...
     ! counter of writings to ptrack hydro output to be indicated in file name
     n_ptrack = n_ptrack + 1  

     PRINT*,' Writing ptrack_hydro.bnr ', n_ptrack
     ...
     hour_out = ihr
      ...
      ! ... define file name ptrack_hydro.bnr
      !IF (n+1 .GE. INT(cloud_t0(1)*itdt/3600.)) THEN
      !hydro_filename = "ptrack_hydro .bnr" 
      IF ( n_ptrack <  10 ) THEN
      !CHARACTER(LEN=17):: hydro_filename
      ptrack_file = "ptrack_hydro .bnr" 
      !PRINT*,' hydro file n = ', n
      WRITE ( ptrack_file(13:13), FMT='(I1)' ) n_ptrack
      ELSE IF (n_ptrack .GE. 10 .AND. n_ptrack .LT. 100 ) THEN
      ptrack_file = "ptrack_hydro  .bnr" 
      WRITE ( ptrack_file(13:14), FMT='(I2)' ) n_ptrack
      ELSE IF (n_ptrack .GE. 100 .AND. n_ptrack .LT. 1000 ) THEN
      ptrack_file = "ptrack_hydro   .bnr" 
      WRITE ( ptrack_file(13:15), FMT='(I3)' ) n_ptrack
      ELSE IF (n_ptrack .GE. 1000 .AND. n_ptrack .LT. 10000 ) THEN
      ptrack_file = "ptrack_hydro    .bnr" 
      WRITE ( ptrack_file(13:16), FMT='(I4)' ) n_ptrack
      END IF

     ! ... Open output file
     OPEN(unit=ptrack_id,file=path//ptrack_file,FORM='UNFORMATTED',IOSTAT=ios)   ! include path of folder where file shoulde be written
     IF(ios /= 0) THEN
       PRINT *, "Error opening ptrack hydro file = ", ios, n_ptrack
       STOP
     END IF
     ...
     DO c=1,cm1
       ...
       DO k = k1, kmz(l)
         ...
         out_array(kout,5) = Dvm(k,l)
         out_array(kout,6) = sal (k,l)
         out_array(kout,7) = s (l)  ! cintia wse
       END DO;
     END DO
     ...
     CASE (-1) 
       ...
       DO innH = 1, iopssH(omp_get_thread_num ( )+1)
	     ...
	     IF (ntr > 0) THEN
           ...
           ENDDO
         ENDIF
       PRINT*,'iopsLoc i: ',ipss(inn),' iopsloc j:',jpss(inn) !cintia 
...
!------------------------------------------------------------------------
   ...
   DO nn = 1, npssdev
      ...
      IF (ios /= 0) CALL input_error ( ios, 48 )
      PRINT *, " ************* Puntos pss.txt: ", nptspss  ! Cintia
      ...
      CASE (-2)     
        ...
        DO j = 1, nptspss
         ...
         IF (ios /= 0) CALL input_error ( ios, 49 )
        END DO
        PRINT *, " primera fila pss.txt: ", varspss(nn,:,1) ! Cintia
