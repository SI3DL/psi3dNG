...
!-------------------------------------------------------------------------
   ...
   INTEGER :: maxcount = 1E6,nn,is,ie,js,je,noH,isH,ieH,la,laaux,contBC,istat
   ...
   !    i = i + 1
   !END DO
   ALLOCATE( out_array ( lm1 * km1 , 11 ), STAT=istat )
   ...
   IF (ipxml < 0 .AND. ptr .EQ. 0) CALL outp(n)   ! Three-dimensional outputs for ptrack single file
   IF (ipxml < 0 .AND. ptr .EQ. 1) CALL outp2(n)  ! Three-dimensional outputs for ptrack file series ! ABH
...
!      print *,"entro en fdfrog",omp_get_thread_num ( )
    ...
    3 IF((nnodes > 0) .AND. (MOD(n,MAX(ipt,  1)) == 0)) CALL outt(n,thrs)
      ...
      IF((ipxml  < 0) .AND. (ptr .EQ. 0) .AND. (n .GE. itspf) .AND. (MOD(n,MAX(apxml,1)) == 0)) CALL outp(n)  ! 2nd condition added by ABH 2012, Jan 27 to write to file (ptrack_hydro.bnr) only after the initial time set in si3d_inp.txt
      IF((ipxml  < 0) .AND. (ptr .EQ. 1) .AND. (n .GE. itspf) .AND. (MOD(n,MAX(apxml,1)) == 0)) CALL outp2(n)  ! 2nd condition added by ABH 2012, Jan 27 to write to file (ptrack_hydro.bnr) only after the initial time set in si3d_inp.txt
      ! -------------------------------------------------------------------------------------------------------------
