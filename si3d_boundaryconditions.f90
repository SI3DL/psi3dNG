...
!------------------------------------------------------------------------
   ...
   IF ( noNGB > 0) THEN
     ...
     ALLOCATE ( uhNGB(maxiptsNB,noNGB), uhNGBp(maxiptsNB,noNGB), &
                ...
                huNGB(maxiptsNB,noNGB), huNGBp(maxiptsNB,noNGB), &
                hvNGB(maxiptsNB,noNGB), hvNGBp(maxiptsNB,noNGB), &
                sNGB(maxiptsNB,noNGB), sNGBp(maxiptsNB,noNGB), &   !MAC NESTING
                agNGB(maxiptsNB,noNGB), agNGBp(maxiptsNB,noNGB), &
                arNGB(maxiptsNB,noNGB), arNGBp(maxiptsNB,noNGB), &
                STAT=istat)
     ...
     DO nn = 1, nopen
       ...
       CASE(1,3)
         DO icl = 1, iptNBI(nn)
           ...
           vhNGBp(icl,niNGB) = inputvar(icl,6) !MAC NESTING
           sNGBp(icl,niNGB) = inputvar(icl,7)  !MAC NESTING
           huNGBp(icl,niNGB) = inputvar(icl,8) !MAC NESTING
           hvNGBp(icl,niNGB) = inputvar(icl,9)  !MAC NESTING
           agNGBp(icl,niNGB) = inputvar(icl,10) !MAC NESTING
           arNGBp(icl,niNGB) = inputvar(icl,11)  !MAC NESTING

           IF (ntr > 0) THEN
             trNGBp(icl,1:ntr,niNGB) = inputvar(icl,12:11+ntr) 
           ENDIF
         ENDDO
       CASE(2,4)
         DO icl = 1, iptNBI(nn)
           ...
           scNGBp(icl,niNGB) = inputvar(icl,5) 
           uhNGBp(icl,niNGB) = inputvar(icl,6) !MAC NESTING
           sNGBp(icl,niNGB) = inputvar(icl,7)  !MAC NESTING
           huNGBp(icl,niNGB) = inputvar(icl,8) !MAC NESTING
           hvNGBp(icl,niNGB) = inputvar(icl,9)  !MAC NESTING
           agNGBp(icl,niNGB) = inputvar(icl,10) !MAC NESTING
...
!       print *,"uhNGBp:",sum(uhNGBp(:,:)),"vhNGBp:",sum(vhNGBp(:,:)),"scNGBp:",sum(scNGBp(:,:))
       ...
       !          ((inputvar(m1,m2),m2=4,11+ntr),m1=1,iptNBI(nn))
       ...
       !    vhNGB(icl,niNGB) = inputvar(icl,6) !MAC NESTING
       !    sNGB(icl,niNGB) = inputvar(icl,7)  !MAC NESTING
       !    huNGB(icl,niNGB) = inputvar(icl,8) !MAC NESTING
       !    hvNGB(icl,niNGB) = inputvar(icl,9)  !MAC NESTING
       !    agNGB(icl,niNGB) = inputvar(icl,10) !MAC NESTING
       !    arNGB(icl,niNGB) = inputvar(icl,11)  !MAC NESTING
       !    IF (ntr > 0) THEN
       !      trNGB(icl,1:ntr,niNGB) = inputvar(icl,12:11+ntr) 
       !    ENDIF
       !  ENDDO
       !CASE(2,4)
       !  DO icl = 1, iptNBI(nn)
       ...
       !    scNGB(icl,niNGB) = inputvar(icl,5)
       !    uhNGB(icl,niNGB) = inputvar(icl,6) !MAC NESTING
       !    sNGB(icl,niNGB) = inputvar(icl,7)  !MAC NESTING
       !    huNGB(icl,niNGB) = inputvar(icl,8) !MAC NESTING
       !    hvNGB(icl,niNGB) = inputvar(icl,9)  !MAC NESTING
       !    agNGB(icl,niNGB) = inputvar(icl,10) !MAC NESTING
...
!       print *,"uhNGB:",sum(uhNGB(:,:)),"vhNGB:",sum(vhNGB(:,:)),"scNGB:",sum(scNGB(:,:))
   ...
   sEB    = 0.0  ; sNB    = 0.0  ;
   sWB    = 0.0  ; sSB    = 0.0  ;
   auxhWB = 0.0  ; auxhNB = 0.0  ;
   auxhEB = 0.0  ; auxhSB = 0.0  ;
   agWB   = 0.0  ; agNB   = 0.0  ;
   agEB   = 0.0  ; agSB   = 0.0  ;
   arWB   = 0.0  ; arNB   = 0.0  ;
   arEB   = 0.0  ; arSB   = 0.0  ;
        ...
        CASE(1)
          ...
          DO j = js, je; 
            ...
            IF ( (.NOT. mask2d(i,j)) .OR. mask2d(i-1,j) ) THEN
              ...
              STOP
            END IF
            DO k = k1, kmz(l) !MAC NESTING
              icl = icl + 1      
              uhWB(k,j) = uhNGBp(icl,niNGB)
              huWB(k,j) = huNGBp(icl,niNGB)
              !huWB(k,j) = hup(k,l)
              vhWB(k,j) = vhNGBp(icl,niNGB)
              !hvWB(k,j) = hp(k,l)
              hvWB(k,j) = hvp(k,l)
              auxhWB(k,j) = hvNGBp(icl,niNGB)
              agWB(k,j) = agNGBp(icl,niNGB)
              arWB(k,j) = arNGBp(icl,niNGB)
              !if(j .EQ. je) THEN
              !     aux = s(lm1)
              !else
              !     aux = sWBp(k,j+1)
              ...
              !auxhWB(k,j)= AMIN1(zlevel(k+1),hhv(l)) -            &
              !     &        AMAX1(zlevel(  k),-MAX(sWBp(k,j),aux))
        ...
        CASE(3)
          ...
          DO j = js, je; 
            ...
            DO k = k1, kmz(l)   !MAC NESTING
              ...
              huEB(k,j) = huNGBp(icl,niNGB)
              !huEB(k,j) = AMIN1(zlevel(k+1),hhu(l)) -            &
              !   &        AMAX1(zlevel(  k),-MAX(s(l),sEBp(k,j)))
              vhEB(k,j) = vhNGBp(icl,niNGB)
              hvEB(k,j) = hvp(k,l)
              auxhEB(k,j) = hvNGBp(icl,niNGB)
              agEB(k,j) = agNGBp(icl,niNGB)
              arEB(k,j) = arNGBp(icl,niNGB)
              !hvEB(k,j) = hvp(k,l)
            ENDDO
          ENDDO
        CASE(2)
          ...
          DO i = is, ie; 
            ...
            DO k = k1, kmz(l)  !MAC NESTING
              ...
              hvNB(k,i) = hvNGBp(icl,niNGB)
              !hvNB(k,i) = AMIN1(zlevel(k+1),hhu(l)) -            &
              !   &        AMAX1(zlevel(  k),-MAX(s(l),sNBp(k,j)))
              uhNB(k,i) = uhNGBp(icl,niNGB)
              huNB(k,i) = hup(k,l)
              auxhNB(k,i) = huNGBp(icl,niNGB)
              agNB(k,j) = agNGBp(icl,niNGB)
              arNB(k,j) = arNGBp(icl,niNGB)
              !huNB(k,i) = hup(k,l)
              !if(i .EQ. ie) THEN
              !     aux = s(lm1)
              !else
              !     aux = sNBp(k,i+1)
              !endif
              !auxhNB(k,i)= AMIN1(zlevel(k+1),hhu(l)) -            &
              !     &       AMAX1(zlevel(  k),-MAX(sNBp(k,i),aux))
            ENDDO
          ENDDO
        CASE(4)
          j   = jsbc(nn); 
          ...
          DO i = is, ie; 
            ...
            DO k = k1, kmz(l)
              ...
              hvSB(k,i) = hvNGBp(icl,niNGB)
              !hvSB(k,i) = hvp(k,l)
              uhSB(k,i) = uhNGBp(icl,niNGB)
              huSB(k,i) = hup(k,l)
              !huSB(k,i) = hup(k,i)
              auxhSB(k,i) = huNGBp(icl,niNGB)
              agSB(k,j) = agNGBp(icl,niNGB)
              arSB(k,j) = arNGBp(icl,niNGB)
              !if(i .EQ. ie) THEN
              !     aux = s(lm1)
              !else
              !     aux = sSBp(k,i+1)
              !endif
              !auxhSB(k,i)= AMIN1(zlevel(k+1),hhu(l)) -            &
              !     &       AMAX1(zlevel(  k),-MAX(sSBp(k,i),aux))
            ENDDO
          ENDDO
        END SELECT   
...
!      print *,"uhWB:",sum(uhWB(:,:)),"huWB:",sum(huWB(:,:))
   ...
   sNBp   = sNB  ; sSBp   = sSB  ;
   sWBp   = sWB  ; sEBp   = sEB  ;
   sNBpp  = sNBp ; sSBpp  = sSBp ;  !MAC NESTING
   sWBpp  = sWBp ; sEBpp  = sEBp ;
   

   uhNBp  = uhNB ; huNBp  = huNB ;
   uhSBp  = uhSB ; huSBp  = huSB ;
   vhEBp  = vhEB ; hvEBp  = hvEB ;
   vhWBp  = vhWB ; hvWBp  = hvWB ;
   uhNBpp = uhNBp; huNBpp = huNBp;
   uhSBpp = uhSBp; huSBpp = huSBp;
   vhEBpp = vhEBp; hvEBpp = hvEBp;
   vhWBpp = vhWBp; hvWBpp = hvWBp;
   auxhNBp   = auxhNB  ; auxhSBp   = auxhSB  ;
   auxhWBp   = auxhWB  ; auxhEBp   = auxhEB  ;
   auxhNBpp  = auxhNBp ; auxhSBpp  = auxhSBp ;  !MAC NESTING
   auxhWBpp  = auxhWBp ; auxhEBpp  = auxhEBp ;
         ...
         CASE (4)
            ...
            DO i = is,ie
              ...
              DO k = k1, kmy
                ...
                IF(hvSB(k,i) <= HMIN) hvSB(k,i) = ZERO;
                !$omp critical
                areatot(no) = areatot(no) + hvSB(k,i) * dy ! cambio cintia hvNB por hvSB
                !$omp end critical
              ENDDO
            ENDDO
            !$omp critical
...
!         print *,"thrsNGB:",thrsNGB,"weight:",weight
         ...
         CASE(1)
           ...
           DO j = js, je;
             ...
             DO k = k1, kmz(l)
               icl = icl + 1    
               sWB (k,j) = sNGB (icl,no)*    weight + &
                           sNGBp(icl,no)*(1.-weight)  
               uhWB(k,j) = uhNGB (icl,no)*    weight + &
                           uhNGBp(icl,no)*(1.-weight)
               huWB(k,j)=  huNGB (icl,no)*    weight + &
                           huNGBp(icl,no)*(1.-weight)
               !huWB(k,j)= hu(k,l)
               !if(j .EQ. je) THEN   !MAC NESTING
               !    aux = s(lm1)
               !else
               !    aux = sWB(k,j+1)
               !endif
               auxhWB(k,j)= hvNGB (icl,no)*    weight + &
                            hvNGBp(icl,no)*(1.-weight)
               
               vhWB(k,j) = vhNGB (icl,no)*    weight + &
                           vhNGBp(icl,no)*(1.-weight)
               hvWB(k,j) = hv(k,l)
               agWB(k,j) = agNGB(icl,no)
               arWB(k,j) = arNGB(icl,no)
               !hvWB(k,j) = hv(k,l)
             ENDDO 
           ENDDO 
         CASE(3)
           ...
           DO j = js, je; 
             ...
             DO k = k1, kmz(l)
               icl = icl + 1      
               sEB (k,j) = sNGB (icl,no)*    weight + &
                           sNGBp(icl,no)*(1.-weight)  
               uhEB(k,j) = uhNGB (icl,no)*    weight + &
                           uhNGBp(icl,no)*(1.-weight)
               huEB(k,j) = huNGB (icl,no)*    weight + &
                           huNGBp(icl,no)*(1.-weight)
               !if(j .EQ. je) THEN   !MAC NESTING
               !    aux = s(lm1)
               !else
               !    aux = sEB(k,j+1)
               !endif
               auxhEB(k,j)= hvNGB (icl,no)*    weight + &
                            hvNGBp(icl,no)*(1.-weight)
               !huEB(k,j) = AMIN1(zlevel(k+1),hhu(l)) -            &
               !   &        AMAX1(zlevel(  k),-MAX(s(l),sEB (k,j)))
               vhEB(k,j) = vhNGB (icl,no)*    weight + &
                           vhNGBp(icl,no)*(1.-weight)
               hvEB(k,j) = hv(k,l)
               agEB(k,j) = agNGB(icl,no)
               arEB(k,j) = arNGB(icl,no)
               !hvEB(k,j) = hv(k,l)
             ENDDO
           ENDDO
         CASE(2)
           ...
           DO i = is, ie; 
             ...
             DO k = k1, kmz(l)
               icl = icl + 1  
               sNB (k,i) = sNGB (icl,no)*    weight + &
                           sNGBp(icl,no)*(1.-weight)      
               vhNB(k,i) = vhNGB (icl,no)*    weight + &
                           vhNGBp(icl,no)*(1.-weight)
               hvNB(k,i) = hvNGB (icl,no)*    weight + &
                           hvNGBp(icl,no)*(1.-weight)
               !if(i .EQ. ie) THEN   !MAC NESTING
               !    aux = s(lm1)
               !else
               !    aux = sNB(k,i+1)
               !endif
               auxhNB(k,i)= huNGB (icl,no)*    weight + &
                            huNGBp(icl,no)*(1.-weight)
               !hvNB(k,i) = AMIN1(zlevel(k+1),hhu(l)) -            &
               !   &        AMAX1(zlevel(  k),-MAX(s(l),sNB (k,j)))
               uhNB(k,i) = uhNGB (icl,no)*    weight + &
                           uhNGBp(icl,no)*(1.-weight)
               huNB(k,i) = hu(k,l)
               agNB(k,i) = agNGB(icl,no)
               arNB(k,i) = arNGB(icl,no)
               !print *,"agNB:",agNB(k,i),"arNB:",arNB(k,i),k,l
               !print *,"vhNB:",vhNB(k,i),"vhNGB:",vhNGB(icl,no),"vhNGBp:",vhNGBp(icl,no)
               !print *,k,l,weight
               !huNB(k,i) = hu(k,l)
             ENDDO
         ...
         CASE(4)
           ...
           DO i = is, ie; 
             ...
             DO k = k1, kmz(l)
               icl = icl + 1   
               sSB (k,i) = sNGB (icl,no)*    weight + &
                           sNGBp(icl,no)*(1.-weight)     
               vhSB(k,i) = vhNGB (icl,no)*    weight + &
                           vhNGBp(icl,no)*(1.-weight)
               !hvSB(k,i) = h(k,l)
               hvSB(k,i) = hvNGB (icl,no)*    weight + &
                           hvNGBp(icl,no)*(1.-weight)
               !if(i .EQ. ie) THEN    !MAC NESTING
               !    aux = s(lm1)
               !else
               ...
               !endif
               !auxhSB(k,i)= AMIN1(zlevel(k+1),hhu(l)) -            &
               !    &        AMAX1(zlevel(  k),-MAX(sSB(k,i),aux))
               !hvSB(k,i) = hv(k,l)
               uhSB(k,i) = uhNGB (icl,no)*    weight + &
                           uhNGBp(icl,no)*(1.-weight)
               huSB(k,i) = hu(k,l)
               auxhSB(k,i) = huNGB (icl,no)*    weight + &
                             huNGBp(icl,no)*(1.-weight)
               agSB(k,i) = agNGB(icl,no)
               arSB(k,i) = arNGB(icl,no)
               !huSB(k,i) = hu(k,l)
             ENDDO
           ENDDO
         END SELECT   
...
!$omp barrier
   ...
   DO nn = 1, nopenH(ide_t) 
     ...
     !IF (thrs > thrsNGB(no)) THEN
       ...
       sNGBp(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no) = sNGB(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no);
       huNGBp(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no) = huNGB(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no); 
       hvNGBp(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no) = hvNGB(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no);
       agNGBp(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no) = agNGB(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no); 
       arNGBp(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no) = arNGB(siptNBIH(no,ide_t):eiptNBIH(no,ide_t),no);
       IF (ntr > 0) THEN 
...
!       trNGBp = trNGB;
       ...
       ! ... Allocate space for temporary input variable array 
       ALLOCATE( inputvar ( iptNBI(no), 11+ntr ), STAT=istat )
       IF (istat /= 0) CALL allocate_error (istat,34)
       ...
       READ(nbiid) auxthrs, & 
           ((inputvar(m1,m2),m2=4,11+ntr),m1=1,iptNBI(no))
       ...
       CASE(1,3)
         DO icl = siptNBI(no,ide_t), eiptNBI(no,ide_t)
           ...
           vhNGB(icl,no) = inputvar(icl,6)
           sNGB(icl,no) = inputvar(icl,7) !MAC NESTING
           huNGB(icl,no) = inputvar(icl,8) 
           hvNGB(icl,no) = inputvar(icl,9)
           agNGB(icl,no) = inputvar(icl,10) 
           arNGB(icl,no) = inputvar(icl,11)
           IF (ntr > 0) THEN
             trNGB(icl,1:ntr,niNGB) = inputvar(icl,12:11+ntr) 
           ENDIF
         ENDDO
       CASE(2,4)
         DO icl = siptNBI(no,ide_t), eiptNBI(no,ide_t)
           ...
           arNGB(icl,no) = inputvar(icl,11)
           IF (ntr > 0) THEN
             trNGB(icl,1:ntr,no) = inputvar(icl,12:11+ntr) 
           ENDIF
         ENDDO
       END SELECT
     ...
     !print *,"******************************"
     !print *,"agNGB:",sum(agNGB(:,1)),"arNGB:",sum(arNGB(:,1))
     !print *,"******************************"
...
!************************************************************************
SUBROUTINE MODmatcon(Bstart,Bend,Beagx,Beagy,Bearx,Beary,Bagx,Bagy,Barx,Bary)
!************************************************************************
!
...
!------------------------------------------------------------------------
   ...
   REAL, DIMENSION (Bstart:Bend+1), INTENT(INOUT) :: Beagx,Bearx,Beagy,Beary
   

   !.....Local variables.....
   INTEGER :: i, j, nn, is, ie, js, je, ks, ke,l,iaux,jaux,k1x,kmx
...
END SUBROUTINE MODmatcon

!************************************************************************
SUBROUTINE MODqqddrr4openBC(Bstart,Bend,Bqq,Brr,Bsx,Bsy,Bdd,Beagx,Beagy,Bearx,Beary,lWCH,lSCH)
!************************************************************************
!
!  Purpose: To adjust the matrix coefficients used in the soln of the
!           continuity equation to account for open boundary conditions,
...
!------------------------------------------------------------------------
   ...
   INTEGER, INTENT(IN) :: Bstart,Bend
   REAL, DIMENSION (Bstart:Bend+1), INTENT(INOUT) :: Bqq,Bsx,Bsy,Bdd,Beagx,Beagy,Bearx,Beary,Brr
   INTEGER, DIMENSION(Bstart:Bend+1), INTENT(IN) :: lWCH,lSCH
   

   !.....Local variables.....
   INTEGER :: i, j, nn, is, ie, js, je, ks, ke,laux,iaux,jaux,l,k1s,kms
   REAL    :: dt1, dtdx1, dtdy1,no,ide_t,cx,rho4sx,rho4sy,cy
      ...
      CASE (2,4)
         ...
         CASE (1)
            ...
            DO j = js, je
               ...
               !Bqq(l) = spp(l) - (dtdx1)*(Beagx(l)-SUM(agWB(:,j)))   &
               !&       - (dtdy1)*(Beagy(l)-Beagy(lSCH(l)))   &
               !&       - Bdd(l)
               Bqq(l) = Bqq(l) + dtdx1*SUM(uhWB  (k1:kmz(l),j)) &
                                     + dtdx1*SUM(uhWBpp(k1:kmz(l),j))
               !Brr(l) = 1 + Bsx(l) + (cx * rho4sx * SUM(arWB(:,j))) + Bsy(l) + Bsy(lSCH(l))
         ...
         CASE (2)
            ...
            DO i = is, ie
               ...
               !&       - (dtdy1)*(SUM(agNB(:,i))-Beagy(lSCH(l)))   &
               !&       - Bdd(l)
               Bqq(l) = Bqq(l) - dtdy1*SUM(vhNB  (k1:kmz(l),i)) &
                                     - dtdy1*SUM(vhNBpp(k1:kmz(l),i))
               !print *,"Beagx:",Beagx(l),"BeagxW:",Beagx(lWCH(l)),"agNB:",SUM(agNB(:,i))
               !print *,"BeagyS:",Beagy(lSCH(l)),"Bdd:",Bdd(l),l
               !Brr(l) = 1 + Bsx(l) + Bsx(lWCH(l)) + (cy * rho4sy * SUM(arNB(:,i))) + Bsy(lSCH(l))
            END DO
         ...
         CASE (3)
            ...
            DO j = js, je
               ...
               k1s = k1
               kms = kmz(l)
               Bdd(l) = dt1*SUM((uhEBpp(k1s:kms,j) -         &
               &         uhpp(k1s:kms,lWC(l))) /dx       &
               &        +(vhpp(k1s:kms,     l) -         &
               &          vhpp(k1s:kms,lSC(l)))/dy)
               !Bqq(l) = spp(l) - (dtdx1)*(SUM(agEB(:,j))-Beagx(lWCH(l)))   &
               !&       - (dtdy1)*(Beagy(l)-Beagy(lSCH(l)))   &
               !&       - Bdd(l)
               Bqq(l) = Bqq(l) - dtdx1*SUM(uhEB  (k1:kmz(l),j)) &
                                     - dtdx1*SUM(uhEBpp(k1:kmz(l),j)) 
               !Brr(l) = 1 + (cx * rho4sx * SUM(arEB(:,j))) + Bsx(lWCH(l)) + Bsy(l) + Bsy(lSCH(l))
            END DO
         ...
         CASE (4)
            ...
            DO i = is, ie
               ...
               &          vhSBpp(k1s:kms,i))/dy)
               !Bqq(l) = spp(l) - (dtdx1)*(Beagx(l)-Beagx(lWCH(l)))   &
               !&       - (dtdy1)*(Beagy(l)-SUM(agSB(:,i)))   &
               !&       - Bdd(l)
               Bqq(l) = Bqq(l) + dtdy1*SUM(vhSB  (k1:kmz(l),i)) &
                                     + dtdy1*SUM(vhSBpp(k1:kmz(l),i))
               !Brr(l) = 1 + Bsx(l) + Bsx(lWCH(l)) + Bsy(l) + (cy * rho4sy * SUM(arSB(:,i)))
            END DO
         END SELECT
...
!------------------------------------------------------------------------
   ...
   !.....Local variables.....
   INTEGER :: i, j, nn, is, ie, js, je, m,no,ide_t,l
   REAL :: cx,cy,dtdx1,dtdy1,dt1,rho4sx,rho4sy
   !.....Constants.....
   cx     = gdt2dx2*tz*tz;
   cy     = gdt2dy2*tz*tz;
   dtdx1  = dtdx*tz;
      ...
      CASE (2,4)
       ...
       ! Identify flow boundary as on the west, north, east, or south
         ...
         CASE (1)
            ...
            DO j = js, je
               l = ij2l(i,j)
               coeffA (l,2) = -(cx * rho4sx * SUM(arWB(:,j)))
            END DO
         ...
         CASE (2)
            ...
            DO i = is, ie
               l = ij2l(i,j)
               coeffA (l,4) = -(cy * rho4sy * SUM(arNB(:,i)))
               !jcoefA (l,4) =  l
            END DO
         ...
         CASE (3)
            ...
            DO j = js, je
               l = ij2l(i,j)
               coeffA (l,5) = -(cx * rho4sx * SUM(arEB(:,j)))
            END DO
         ...
         CASE (4)
            ...
            DO i = is, ie
               l = ij2l(i,j)
               coeffA (l,3) = -(cy * rho4sy * SUM(arSB(:,i)))
            END DO
         END SELECT   
...
!------------------------------------------------------------------------
   ...
   REAL :: advx, advy, uE, uW, vN, vS, wU, wD, &
           ubdry, vbdry, uhbdry, vhbdry, corx, hdx, aux, sumcorx, sumhdx, sumadvx
     ...
     CASE (2,4)
      ...
      ! Identify wse boundary as on the west, north, east, or south
       ...
       CASE (1)
         ...
         DO j = js, je
           ...
           DO k = k1x,kmx
            ...
            !.....Coriolis.....
            corx = 0.25 * f * (vhp(k,     lEC(l) ) + vhp(k,    l )       & !MAC NESTING
       ...
       DO i = is, ie
           ...
           !kmx = kmz(l)                    !MAC NESTING
           !DO k = k1, kmx
           !   IF(hvNBp(k,i) > ZERO) THEN
           !      k1v(l) = k
           !      EXIT
           !   ENDIF
           !ENDDO
           ...
           DO k = k1x,kmx
            ...
            if(i .EQ. ie) THEN
                ...
                !aux = (vhNB(k,i+1)+vhNBpp(k,i+1))/2.
                aux = vhNBp(k,i+1)
            end if               
            !.....Coriolis.....
            corx = 0.25 * f * (aux + vhbdry       &  !MAC NESTING
                  &           +vhp(k, lSC(lEC(l))) + vhp(k,lSC(l)))
                                        
            ! Horizontal advection - Upwind differencing  
            uhbdry = (uhNB(k,i)+uhNBpp(k,i))/2.
            IF (auxhNBpp(k,i)>ZERO) THEN
               ubdry  = uhNBpp(k,i)/auxhNBpp(k,i)   !MAC NESTING
            ELSE
               ubdry  = 0.0
            ENDIF
            uE = uhp(k,    lEC(l) ) +uhp(k  ,    l )
            ...
            haypp(k,l)=Ay0*MIN(auxhNBpp(k,i),hupp(k,l))         !MAC NESTING
            hdx  =   (haypp(k,    l )*(ubdry-upp(k,    l ))       &
                    & -  haypp(k,lSC(l))*(upp(k,    l )-upp(k,lSC(l))))/dydy &
                    & + (haxpp(k,    l )*(upp(k,lEC(l))-upp(k,    l ))       &
                    & -  haxpp(k,lWC(l))*(upp(k,    l )-upp(k,lWC(l))))/dxdx
            !if(j .EQ. jsbc(1) .AND. i <= iebc(1) .AND. i >= isbc(1)) THEN
                !print *,"aqui no entro"
                !print *,"aadvx:",advx
                !print *,"iiadv:",iadv
                !print *,"ccorx:",corx
                !print *,"hhdx:",hdx
                !sumadvx=sumadvx+advx
       ...
       CASE (3)
         ...
         DO j = js, je
           ...
           DO k = k1x,kmx
            ...
            haxpp(k,l)=Ax0*MIN(huEBpp(k,j),hupp(k,l))       !MAC NESTING
            hdx  =   (haypp(k,    l )*(upp(k,lNC(l))-upp(k,    l ))       &
                    & -  haypp(k,lSC(l))*(upp(k,    l )-upp(k,lSC(l))))/dydy &
                    & + (haxpp(k,    l )*(ubdry-upp(k,    l ))       &
                    & -  haxpp(k,lWC(l))*(upp(k,    l )-upp(k,lWC(l))))/dxdx
       ...
       j  = jsbcH(no,ide_t); 
         ...
         DO i = is, ie
           ...
           DO k = k1x,kmx
            ...
            ! Horizontal advection - Upwind differencing  
            uhbdry = (uhSB(k,i)+uhSBpp(k,i))/2.
            IF (auxhSBpp(k,i)>ZERO) THEN
               ubdry  = uhSBpp(k,i)/auxhSBpp(k,i)
            ELSE
               ubdry  = 0.0
            ENDIF
            ...
            aux=Ay0*MIN(auxhSBpp(k,i),hupp(k,l))		      		
            hdx  =   (haypp(k,    l )*(upp(k,lNC(l))-upp(k,    l ))       &
                    & -  aux*(upp(k,    l )-ubdry))/dydy &
                    & + (haxpp(k,    l )*(upp(k,lEC(l))-upp(k,    l ))       &
                    & -  haxpp(k,lWC(l))*(upp(k,    l )-upp(k,lWC(l))))/dxdx 
...
!------------------------------------------------------------------------
   ...
   REAL :: twodt1
   REAL :: advx, advy, uE, uW, vN, vS, wU, wD, &
           ubdry, vbdry, uhbdry, vhbdry, hdy, cory, aux
   INTEGER :: i, j, k, l, istat, kmx, kmy, k1x, k1y, k1ne
   INTEGER :: nn, is, ie, js, je, ks, ke, nwlayers,no,ide_t
   REAL :: sumadvx,sumcorx,sumhdx
     ...
     CASE (2,4)
       ...
       i  = isbcH(no,ide_t); 
         ...
         DO j = js, je
           ...
           ex(:,l) = 0.0	     
           DO k = k1y,kmy
            !uhbdry = (uhWB(k,j)+uhWBpp(k,j))/2.        !MAC NESTING 
            uhbdry = uhWBp(k,j)               
            if(j .EQ. je) THEN
                aux = uhp(k,lm1)
            else
                !aux = (uhWB(k,j+1)+uhWBpp(k,j+1))/2. 
                 aux = uhWBp(k,j+1)  
            end if                                
            ! Horizontal advection Upwind differencing 
            vhbdry = (vhWB(k,j)+vhWBpp(k,j))/2.
            IF (auxhWBpp(k,j)>ZERO) THEN
               vbdry  = vhWBpp(k,j)/auxhWBpp(k,j)
            ELSE
               vbdry  = 0.0
            ENDIF
            uE = uhp(k,    lNC(l) ) +uhp(k,    l )
            uW = aux +uhbdry
            vN = vhp(k,        l  ) +vhp(k,lNC(l))
            vS = vhp(k,        l  ) +vhp(k,lSC(l))
            wU = wp (k,    lNC(l) ) +wp (k,    l )
            wD = wp (k+1,  lNC(l) ) +wp (k+1  ,l )        
            advy = ( (uE+ABS(uE))* vpp(k,    l ) +           &
       ...
       CASE (2)
         ...
         DO i = is, ie
           ...
           DO k = k1y,kmy
            ...
            haypp(k,l)=Ay0*MIN(hvNBpp(k,i),hvpp(k,l))   
            hdy  =   (haypp(k,    l )*(vbdry-vpp(k,    l ))              &
                   & -  haypp(k,lSC(l))*(vpp(k,    l )-vpp(k,lSC(l))))/dydy        &
                   & + (haxpp(k,    l )*(vpp(k,lEC(l))-vpp(k,    l ))              &
                   & -  haxpp(k,lWC(l))*(vpp(k,    l )-vpp(k,lWC(l))))/dxdx

            !sumadvx=sumadvx+advy
                !sumcorx=sumcorx+cory
                !sumhdx=sumhdx+hdy                   
            !.....Final explicit term.....
            ex(k,l) = vhpp(k,l) - twodt1*(advy*iadv+cory-hdy)
            !print *,"vhNB:",vhNB(k,i),"vhNBpp:",vhNBpp(k,i),"hvNBpp:",hvNBpp(k,i)
            !print *,k,l
                
           END DO
           
                !print *,"aqui no entro"
                !print *,"vhpp:",sum(vhpp(:,l)),i,j
                !print *,"advx:",sumadvx
                !print *,"iadv:",iadv
                !print *,"cory:",sumcorx
                !print *,"hdy:",sumhdx
       ...
       i  = isbcH(no,ide_t); 
         ...
         DO j = js, je
           ...
           ! Compute the layer number for the bottom wet u-pt
           !kmy = kmz(l)                          !MAC NESTING
           !DO k = k1, kmy
           !    IF(huEBp(k,j) > ZERO) THEN
           !       k1u(l) = k
           !       EXIT
           !    ENDIF
           !ENDDO
           ...
           ex(:,l) = 0.0	     
           DO k = k1y,kmy
            !uhbdry = (uhEB(k,j)+uhEBpp(k,j))/2.        !MAC NESTING  
            uhbdry = uhEBp(k,j)              
            if(j .EQ. je) THEN
                aux = uhp(k,lm1)
            else
                !aux = (uhEB(k,j+1)+uhEBpp(k,j+1))/2.
                aux = uhEBp(k,j+1)   
            end if                                                       
            ! Horizontal advection Upwind differencing 
            vhbdry = (vhEB(k,j)+vhEBpp(k,j))/2.
            IF (auxhEBpp(k,j)>ZERO) THEN
               vbdry  = vhEBpp(k,j)/auxhEBpp(k,j)
            ELSE
               vbdry  = 0.0
            ENDIF
            uE = uhp(k,    lNC(l) ) +uhp(k,    l )
            ...
            cory = 0.25 * f * (aux + uhbdry       &   !MAC NESTING
                  &           +uhp(k, lWC(lNC(l))) + uhp(k,lWC(l)))

            haxpp(k,l)=Ax0*MIN(hvpp(k,l),auxhEBpp(k,j))
            hdy  =   (haypp(k,    l )*(vpp(k,lNC(l))-vpp(k,    l ))              &
                   & -  haypp(k,lSC(l))*(vpp(k,    l )-vpp(k,lSC(l))))/dydy        &
                   & + (haxpp(k,    l )*(vbdry-vpp(k,    l ))              &
                   & -  haxpp(k,lWC(l))*(vpp(k,    l )-vpp(k,lWC(l))))/dxdx
       ...
       CASE (4)
         ...
         DO i = is, ie
           ...
           DO k = k1y,kmy
            ...
            aux=Ay0*MIN(hvSBpp(k,i),hvpp(k,l)) 
            hdy  =   (haypp(k,    l )*(vpp(k,lNC(l))-vpp(k,    l ))              &
                   & -  aux*(vpp(k,    l )-vbdry))/dydy        &
                   & + (haxpp(k,    l )*(vpp(k,lEC(l))-vpp(k,    l ))              &
                   & -  haxpp(k,lWC(l))*(vpp(k,    l )-vpp(k,lWC(l))))/dxdx
...
!************************************************************************
SUBROUTINE MODmatmom4openBCX(Bstart,Bend,Beagx,Bearx,Bagx,Barx) 
!************************************************************************
!
!  Purpose: !MAC NESTING
!
!------------------------------------------------------------------------
   ...
   INTEGER, INTENT(IN) :: Bstart, Bend
   REAL, DIMENSION (1:km1,Bstart:Bend+1), INTENT(INOUT) :: Bagx,Barx
   REAL, DIMENSION (Bstart:Bend+1), INTENT(INOUT) :: Beagx,Bearx

   !.....Local variables.....
   INTEGER :: i, j, k, l, kmx, k1x, nwlayers, inn, liter, innH
   INTEGER :: nn,no,ide_t,js,je,is,ie
   REAL    :: twodt1, wsx0, tausx, taubx,     &
              hpmin, usurf, vsurf,Uhvalue, Usource,      &
              cwx,lol,vbdry, vbdryE
   REAL :: aaux,aaux2,aaux3
   REAL, DIMENSION(km1) ::  vdiffx,         &
         &  deltaz, Avxdudz, Avx,   &
         & rhopx, gg, hh, ar, ag
   REAL,DIMENSION(km1) :: hvpdrho,bclncx,hupdrho
   ...
   DO nn = 1, nopenHH(ide_t)
     ...
     CASE (2,4)
           ...
           ! ... Compute the layer number for top & bottom wet u-pt
            ...
            CASE (1:)
              ...
              IF (kmx > k1x) THEN   ! Two or more wet layers
                ...
                CALL bclnc_kmNBC (l, kmx, 1, hupdrho(kmx) )
            ...
            IF (hvNBp(k1v(l),i)>ZERO) THEN
               vbdry  = vhNBp(k1v(l),i)/hvNBp(k1v(l),i)
            ELSE
               vbdry  = 0.0
            ENDIF
            IF (hvNBp(k1v(l),i+1)>ZERO) THEN
               vbdryE  = vhNBp(k1v(l),i+1)/hvNBp(k1v(l),i+1)
            ELSE
               vbdryE  = 0.0
            ENDIF
            usurf = up(k1x,l)
            ...
            taubx = cd*SQRT((uhpp(kmx,     l)*uhpp(kmx,        l)) +          &
                  &  ((0.25*(vhNBpp(kmx,i+1)+vhNBpp(kmx,i)             &
            ...
            !if(j .EQ. jsbc(1) .AND. i <= iebc(1) .AND. i >= isbc(1)) THEN
                !print *,"aqui no entro"
            !    print *,"ex:",sum(ex(k1x:kmx,l)),i,j
            !    print *,"hup:",sum(hup(k1x:kmx,l))
            !    print *,"hh:",sum(hh(k1x:kmx))
            !    print *,"gg:",sum(gg(k1x:kmx))
            !    print *,"bclncx:",sum(bclncx(k1x:kmx))
            !    print *,"vdiffx:",sum(vdiffx(k1x:kmx))
            !    print *,"wsx0:",wsx0
            !    print *,"tausx:",tausx
            !    print *,"cwx:",cwx
            !    print *,"rhopx(k1x):",rhopx(k1x)
            !    print *,"uair(l):",uair(l)
            !    print *,"aa2:",sum(aa(2,k1x:kmx))
            ...
            CASE (2:)   ! Two or more wet layers
              CALL tridNBC ( aa, gg, hh, ag, ar, k1x, kmx, kmx+1, nwlayers )
            END SELECT
           ...
           ! ... Compute the layer number for top & bottom wet u-pt
            ...
            CASE (2:)   ! Two or more wet layers (hu->hup)
              ...
              aa(2,k1x:kmx)  =  1.0                                           &
                        ...
                        -(hup(k1x+1:kmx+1,l)/hup(k1x:kmx,l))*aa(3,k1x:kmx)
            END SELECT
            IF (hvSBp(k1v(l),i)>ZERO) THEN
               vbdry  = vhSBp(k1v(l),i)/hvSBp(k1v(l),i)
            ELSE
               vbdry  = 0.0
            ENDIF
            IF (hvSBp(k1v(l),i+1)>ZERO) THEN
               vbdryE  = vhSBp(k1v(l),i+1)/hvSBp(k1v(l),i+1)
            ELSE
               vbdryE  = 0.0
            ENDIF
            ! ... Top boundary conditions......................................
      ...
      END SELECT
   END DO
END SUBROUTINE MODmatmom4openBCX 

!************************************************************************
SUBROUTINE MODmatmom4openBCY(Bstart,Bend,Beagy,Beary,Bagy,Bary) 
!************************************************************************
!
!  Purpose: !MAC NESTING
!
!------------------------------------------------------------------------
   ...
   INTEGER, INTENT(IN) :: Bstart, Bend
   REAL, DIMENSION (1:km1,Bstart:Bend+1), INTENT(INOUT) :: Bagy,Bary
   REAL, DIMENSION (Bstart:Bend+1), INTENT(INOUT) :: Beagy,Beary

   !.....Local variables.....
   INTEGER :: i, j, k, l, kmy, k1y, nwlayers, inn, liter, innH
   INTEGER :: nn,no,ide_t,js,je,is,ie
   REAL    :: twodt1, wsy0, tausy, tauby,     &
              hpmin, usurf, vsurf,Uhvalue, Usource,      &
              cwy,lol, ubdry, ubdryE, ubdryN
   REAL :: aaux,aaux2,aaux3
   REAL, DIMENSION(km1) ::  vdiffy,         &
         &  deltaz, Avydvdz, Avy,   &
         & rhopy, gg, hh, ar, ag
   REAL,DIMENSION(km1) :: hvpdrho,bclncy,hupdrho
   REAL, DIMENSION(3,km1) :: aa
   REAL, PARAMETER :: rhop0 = 1000.
     ...
     CASE (2,4)
      ...
      ! Identify wse boundary as on the west, north, east, or south
           ...
           ! ... Compute layer numbers of wet v-pt
            ...
            CASE (1:)
              ...
              IF (kmy > k1y) THEN   ! Two or more wet layers
                ! Recompute bottom layer baroclinic term along horizontal plane
                CALL bclnc_kmNBC (l, kmy, 2, hvpdrho(kmy) )
                DO k = k1y+1, kmy
            ...
            IF (huWBp(k1u(l),j)>ZERO) THEN
               ubdry  = uhWBp(k1u(l),j)/huWBp(k1u(l),j)
            ELSE
               ubdry  = 0.0
            ENDIF
            IF (huWBp(k1u(l),j+1)>ZERO) THEN
               ubdryE  = uhWBp(k1u(l),j+1)/huWBp(k1u(l),j+1)
            ELSE
               ubdryE  = 0.0
            ENDIF
            ...
            tauby = cd*SQRT((vhpp(kmy,l)*vhpp(kmy,l)) +                       &
                  &  ((0.25*(uhpp(kmy,lNC(l))+uhWBpp(kmy,j+1)           &
                  &         +uhWBpp(kmy,j)+uhpp(kmy,l)))**2))              &
                  &         /(hvp(kmy,l)*hvp(kmy,l))
            ! b. Modify [aa] matrix
            ...
            CASE (2:)   ! Two or more wet layers
              CALL tridNBC ( aa, gg, hh, ag, ar, k1y, kmy, km1, nwlayers )
            END SELECT
       ...
       DO i = is, ie
           ...
           ! ... Cycle if N-column is dry
           IF ( .NOT. mask2d(i,j+1) ) CYCLE
           !print *,"Beagy:",Beagy(l),"Beary:",Beary(l),l
       END DO
       ...
       CASE (3)
         ...
         DO j = js, je
           ...
           ! ... Compute layer numbers of wet v-pt
            ...
            IF (huEBp(k1u(l),j)>ZERO) THEN
               ubdry  = uhEBp(k1u(l),j)/huEBp(k1u(l),j)
            ELSE
               ubdry  = 0.0
            ENDIF
            IF (huEBp(k1u(l),j+1)>ZERO) THEN
               ubdryN  = uhEBp(k1u(l),j+1)/huEBp(k1u(l),j+1)
            ELSE
               ubdryN  = 0.0
            ENDIF
            ...
            ! a. Form bottom stress term
            tauby = cd*SQRT((vhpp(kmy,l)*vhpp(kmy,l)) +                       &
                  &  ((0.25*(uhEBpp(kmy,j+1)+uhpp(kmy,lWC(lNC(l)))           &
                  &         +uhpp(kmy,lWC(l))+uhEBpp(kmy,j)))**2))              &
                  &         /(hvp(kmy,l)*hvp(kmy,l))
            ! b. Modify [aa] matrix
            aa(2,kmy) = aa(2,kmy) + tauby*twodt1
      ...
      END SELECT
   END DO
END SUBROUTINE MODmatmom4openBCY
...
!------------------------------------------------------------------------
   ...
   CASE (2)
     ...
     PRINT*, "*********** puntos surfbc.txt : ",npSurfbc 
     ...
     END DO
     PRINT *, "Primera fila surfbc.txt : ",surfbc1(:,1)
...
!***********************************************************************
SUBROUTINE bclnc_kmNBC (l, kb, ieq, huvpdrho )
!***********************************************************************
!
...
END SUBROUTINE bclnc_kmNBC

!***********************************************************************
SUBROUTINE tridNBC ( acoef, g, r, ag, ar, k1, km, km1, n )
!***********************************************************************
!
